...
def txt_to_pdf(txt_path, pdf_path):
    ...
    width, height = letter
    y = height - 50
    # Batch write 40 chars per line for speed
...
def docx_to_pdf_fast(docx_path, pdf_path):
    doc = Document(docx_path)
    text = "\n".join([p.text for p in doc.paragraphs])
    # Reuse txt_to_pdf logic
    temp_txt = pdf_path.replace(".pdf", ".txt")
    with open(temp_txt, "w", encoding="utf-8") as f:
        f.write(text)
    return txt_to_pdf(temp_txt, pdf_path)

def convert_to_pdf_fast(file_path):
    base_name = os.path.splitext(file_path)[0]
    pdf_path = f"{base_name}.pdf"
    if file_path.lower().endswith('.txt'):
        return txt_to_pdf(file_path, pdf_path)
    elif file_path.lower().endswith('.docx'):
        return docx_to_pdf_fast(file_path, pdf_path)
    elif file_path.lower().endswith('.pdf'):
        return file_path
    else:
...
def call_azure_document_intelligence(file_path):
    ...
    endpoint = "https://recruitfastaidocumentintelligence.cognitiveservices.azure.com/"
    key = "xyz"
...
def extract_skills_from_resume(file_path):
    ...
    try:
        pdf_path = convert_to_pdf_fast(file_path)
...
def extract_skills_from_jd(job_description):
    ...
    try:
        ...
        pdf_path = convert_to_pdf_fast(temp_file)
        response = call_azure_document_intelligence(pdf_path)
        extracted_data = parse_document_intelligence_response(response)
